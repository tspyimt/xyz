...
exports.verifyBearerToken = function (token, done) {
    ...
    if (tokenData && tokenData.e > +new Date()) done(null, {_id: tokenData.i, roles: Role.fromShortNotationToRolesArray(tokenData.r)}, { scope: tokenData.s });
...
exports.localToBearerStrategyMiddleWare = function (req, res, next) {
    ...
    //Create user session
    res.loginUser = function (id, roles) {
    ...
    if (req.cookies[_config.statelessCookieName]) {
        exports.verifyBearerToken(req.cookies[_config.statelessCookieName], function (err, data) {
            if (data) {
                ...
                res.loginUser(data._id, data.roles);
            }
        });
    }
...
exports.resolveContentTypeByExtension = function (extension) {
    extension.replace(/([^\.]+$)/, function () {
        extension = arguments[1]
    });
    return require("./MimeTypes.js").findByExtension(extension);
};

exports.parseFilePath = function (filePath) {
    var data = {};
    filePath.replace(/^(.*?)(([^\/\.]+)\.([^\.]+))$/, function (all, path, fileName, fileNameWithoutExtension, extension) {
        data = {
            all: all,
            path: path,
            fileName: fileName,
...
exports.isArtist = function (user) {
    return (JSON.stringify(user.roles).search(new RegExp(Role.ARTIST)) != -1);
};
